...
[Attribution](./ATTRIBUTION.md): this repository is based on `metatube-community/metatube-plex-plugins` and uses `metatube-community/metatube-sdk-go` for the Go standalone provider.
...
Plex 1.43.0 introduced the new Custom Metadata Providers API. This repository now includes a standalone Go provider under [`provider-go/`](./provider-go/README_ZH.md) that embeds the MetaTube scraping engine directly, plus an older Python prototype under [`provider/`](./provider/README.md).
